...
export const executePlaywrightCommand = async (
  ...
  try {
    ...
    if (cleanCommand.startsWith("await page.goto(")) {
      const match: RegExpMatchArray | null = cleanCommand.match(
        /page\.goto\(['"`]([^'"`]+)['"`](?:,\s*{.*})?\)/,
      );
      if (match) {
    ...
    if (
      cleanCommand.startsWith("page.waitForSelector(") ||
      cleanCommand.startsWith("await page.waitForSelector(")
    ) {
      const match: RegExpMatchArray | null = cleanCommand.match(
    ...
    if (
      cleanCommand === "page.screenshot()" ||
      cleanCommand === "await page.screenshot()"
    ) {
      try {
    ...
    if (
      cleanCommand === "page.title()" ||
      cleanCommand === "await page.title()"
    ) {
    ...
    if (
      cleanCommand.startsWith("page.evaluate(") ||
      cleanCommand.startsWith("await page.evaluate(")
    ) {
      const match: RegExpMatchArray | null = cleanCommand.match(
        /page\.evaluate\(\(\)\s*=>\s*(.+)\)/,
      );
    ...
    if (
      cleanCommand.startsWith("page.keyboard.press(") ||
      cleanCommand.startsWith("await page.keyboard.press(")
    ) {
      const match: RegExpMatchArray | null = cleanCommand.match(
        /page\.keyboard\.press\(['"`]([^'"`]+)['"`]\)/,
      );
  ...
  for (const port of ports) {
    try {
      const { stdout } = await execAsync(
        `docker ps --format "{{.ID}}" --filter "publish=${port}"`,
      );
      if (stdout.trim()) {
